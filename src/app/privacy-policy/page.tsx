import privacyPolicy from "@/context/privacyPolicies";

export default function PrivacyPolicyPage() {
  return (
    <main className="w-full overflow-hidden bg-slate-50 text-slate-900">
      <section className="w-full bg-[linear-gradient(180deg,#ecfeff_0%,#f8fafc_48%,#ffffff_100%)] px-4 py-12 md:px-10 lg:px-16 xl:px-24">
        <div className="mx-auto grid w-full max-w-7xl gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-end">
          <div className="space-y-5">
            <p className="inline-flex w-fit rounded-full border border-cyan-200 bg-white px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.2em] text-cyan-700 sm:text-xs">
              Legal
            </p>
            ...
            <p className="max-w-3xl text-sm leading-relaxed text-slate-700 md:text-base lg:text-lg">
              Effective Date: {privacyPolicy.effectiveDate.trim()}
            </p>
          </div>
        </div>
      </section>
          ...
          {privacyPolicy.sections.map((section, index) => (
            ...
            >
              <div className="flex items-start gap-4">
                <div className="mt-1 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-cyan-100 text-xs font-bold text-cyan-700">
                  {String(index + 1).padStart(2, "0")}
                </div>
                <div className="min-w-0">
                  <h2 className="text-lg font-semibold text-cyan-800 md:text-2xl">
