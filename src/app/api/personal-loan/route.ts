...
export async function POST(req: NextRequest) {
  try {
    const {
      ...
      employmentType, loanPurpose, message,
    } = await req.json();

    // Validation
    if (!fullName || !mobile || !email || !city || !loanAmount || !employmentType || !loanPurpose) {
    ...
    await db.query(
      `INSERT INTO personal_loans
        (full_name, mobile, email, city, loan_amount, employment_type, loan_purpose, message)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [fullName, mobile, email, city, loanAmount, employmentType, loanPurpose, message ?? ""]
    );
    ...
    await transporter.sendMail({
      ...
      subject: `New Personal Loan Application from ${fullName}`,
      html: `
        <div style="font-family:sans-serif;max-width:540px;margin:auto;border:1px solid #e2e8f0;border-radius:12px;overflow:hidden">
          ...
          <div style="padding:24px;background:#f8fafc">
            <table style="width:100%;border-collapse:collapse;font-size:14px">
              ...
              <tr><td style="padding:8px 0;color:#64748b">Loan Purpose</td><td style="padding:8px 0;color:#0f172a">${loanPurpose}</td></tr>
          ...
          <div style="padding:16px 24px;background:#f1f5f9;font-size:12px;color:#94a3b8">Sent via CapitalCare Personal Loan Form</div>
        </div>
      `,
    });
