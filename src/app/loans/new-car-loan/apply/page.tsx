...
import { DOCUMENT_REQUIREMENTS, getDocumentsByCategory, CarLoanApplicantType } from "./documents-config";
...
export default function NewCarLoanApplyPage() {
  const [step, setStep] = useState(1);
  const [applicantType, setApplicantType] = useState<CarLoanApplicantType | "">("");
  ...
  const handleApplicantSelect = (value: CarLoanApplicantType) => {
  ...
  const handleNextStep = () => {
    ...
    if (step === 5) {
      ...
      if (!formData.carMakeModel.trim()) {
        setError("Car make/model is required.");
        return;
      }
      if (!formData.quotationAmount.trim()) {
        setError("Quotation amount is required.");
        return;
      }
  ...
  const handleSubmitApplication = async () => {
    ...
    try {
      ...
      payload.append("loanType", "new-car-loan");
      ...
      console.error("[new-car-loan/apply] submit error", submitError);
      ...
      <div className="mx-auto max-w-4xl">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
          <div className="mb-8 space-y-2">
            <h1 className="text-xl font-bold tracking-tight text-slate-900 md:text-2xl">New Car Loan</h1>
            <p className="text-sm text-slate-600 mb-6">
              Finance your new car purchase with a guided document flow and quotation-based application
            </p>
            <div className="pt-2">
            ...
            <div className="space-y-5">
              ...
              <div className="grid gap-3 md:grid-cols-2">
                {[
                  ...
                  <button
                    key={opt.value}
                    onClick={() => handleApplicantSelect(opt.value as CarLoanApplicantType)}
          ...
          {step === 5 && (
            <div className="space-y-6">
              <div>
                <h2 className="text-lg font-bold text-gray-900">Car and Quotation Details</h2>
                <p className="mt-1 text-sm text-gray-600">PAN, car model, and quotation amount</p>
              </div>
              ...
              <div>
                <label className="mb-2 block text-sm font-semibold text-gray-700">Car Make / Model</label>
                <input
                  value={formData.carMakeModel}
                  onChange={(e) => updateField("carMakeModel", e.target.value)}
                  placeholder="Example: Maruti Suzuki Baleno"
                  className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
              </div>

              <div>
                <label className="mb-2 block text-sm font-semibold text-gray-700">Quotation Amount</label>
                <input
                  value={formData.quotationAmount}
                  onChange={(e) => updateField("quotationAmount", e.target.value.replace(/[^0-9]/g, ""))}
                  inputMode="numeric"
                  placeholder="Car payment quotation amount"
                  className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
              </div>
          ...
          {step === 7 && (
            <div className="space-y-6 py-8 text-center">
              ...
              <div>
                ...
                <p className="mt-2 text-sm text-gray-600">Your new car loan request has been submitted successfully.</p>
                {applicationId && (
