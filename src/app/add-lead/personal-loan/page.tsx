...
const EMPLOYMENT_TYPES = ["Salaried", "Self-Employed", "Business Owner", "Freelancer", "Government Employee", "Other"];
const LOAN_PURPOSES = ["Medical Emergency", "Home Renovation", "Education", "Travel", "Wedding", "Debt Consolidation", "Other"];

type Form = {
  fullName: string; mobile: string; email: string;
  loanAmount: string; employmentType: string;
  loanPurpose: string; city: string; message: string;
};
  ...
  return (
    <div className="mx-auto w-full max-w-2xl px-4 py-12 md:px-6">
      <motion.div
        ...
        className="mb-8 overflow-hidden rounded-3xl border border-indigo-100 bg-linear-to-br from-white via-indigo-50 to-violet-50 px-8 py-10 text-center shadow-[0_12px_40px_-20px_rgba(99,102,241,0.25)]"
      >
        <div className="inline-flex items-center gap-2 rounded-full border border-indigo-200 bg-indigo-50 px-4 py-1.5 text-xs font-semibold uppercase tracking-widest text-indigo-700">
        ...
        <p className="mx-auto mt-3 max-w-md text-sm text-slate-500">
          Instant funds for personal needs with minimal documentation. We&apos;ll respond within 24 hours.
        </p>
      </motion.div>
            ...
            >
              ...
              <div className="mt-2 flex gap-3">
                <button onClick={() => setSubmitted(false)}
                  className="rounded-xl border border-indigo-200 bg-indigo-50 px-5 py-2 text-sm font-semibold text-indigo-700 hover:bg-indigo-100">
                  Apply Again
                </button>
                <Link href="/add-lead" className="rounded-xl bg-linear-to-r from-sky-400 to-indigo-500 px-5 py-2 text-sm font-semibold text-white hover:brightness-110">
                  Back to Loans
                </Link>
              </div>
            </motion.div>
            ...
            >
              <div className="grid gap-5 sm:grid-cols-2">
                <Field label="Full Name" icon={HiOutlineUser} error={errors.fullName}>
                  <input type="text" value={form.fullName} onChange={(e) => set("fullName", e.target.value)} placeholder="Enter your name" className={inputCls} />
                </Field>
              ...
              <Field label="Email Address" icon={HiOutlineMail} error={errors.email}>
                <input type="email" value={form.email} onChange={(e) => set("email", e.target.value)} placeholder="Enter your email" className={inputCls} />
              </Field>
              <div className="grid gap-5 sm:grid-cols-2">
                <Field label="Loan Amount (₹)" icon={HiOutlineCurrencyRupee} error={errors.loanAmount}>
                  <input type="text" value={form.loanAmount} onChange={(e) => set("loanAmount", e.target.value.replace(/\D/g, ""))} placeholder="300000" className={inputCls} />
                </Field>
                <div className="grid gap-1.5">
                  <label className="text-sm font-medium text-slate-700">Loan Purpose</label>
                  <div className="relative">
                    <HiOutlineCurrencyRupee className="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
                    <select value={form.loanPurpose} onChange={(e) => set("loanPurpose", e.target.value)} className={inputCls + " appearance-none"}>
                      <option value="">Select purpose</option>
                      {LOAN_PURPOSES.map((p) => <option key={p} value={p}>{p}</option>)}
                    </select>
                  </div>
                  {errors.loanPurpose && <p className="text-xs text-red-500">{errors.loanPurpose}</p>}
                </div>
              </div>
              <div className="grid gap-5 sm:grid-cols-2">
                <div className="grid gap-1.5">
                  ...
                  <div className="relative">
                    ...
                    <select value={form.employmentType} onChange={(e) => set("employmentType", e.target.value)} className={inputCls + " appearance-none"}>
              ...
              <div className="flex items-center gap-2 rounded-xl border border-indigo-100 bg-indigo-50 px-4 py-2.5">
                <HiOutlineShieldCheck className="h-4 w-4 shrink-0 text-indigo-500" />
              ...
              <motion.button type="submit" disabled={loading}
                ...
                className="rounded-xl bg-linear-to-r from-indigo-400 to-violet-500 py-3 text-sm font-semibold text-white hover:brightness-110 disabled:opacity-60"
              >
                {loading ? <span className="flex items-center justify-center gap-2"><svg className="h-4 w-4 animate-spin" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z" /></svg>Submitting…</span> : "Submit Application"}
              </motion.button>
            </motion.form>
          )}
        </AnimatePresence>
