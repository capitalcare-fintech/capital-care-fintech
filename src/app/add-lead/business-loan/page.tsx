...
import {
  ...
  HiOutlineShieldCheck, HiCheckCircle, HiOutlineOfficeBuilding,
} from "react-icons/hi";
...
type Form = {
  ...
  loanAmount: string; businessType: string;
  annualTurnover: string; city: string; message: string;
};
...
export default function BusinessLoanPage() {
  const [form, setForm] = useState<Form>({
    ...
    loanAmount: "", businessType: "",
    annualTurnover: "", city: "", message: "",
  });
  ...
  function validate(): boolean {
    ...
    if (!form.annualTurnover.trim()) e.annualTurnover = "Annual turnover is required";
    ...
    <div className="mx-auto w-full max-w-2xl px-4 py-12 md:px-6">
      ...
      >
        ...
        <p className="mx-auto mt-3 max-w-md text-sm text-slate-500">
          Fast approvals for growing businesses. Fill in your details and we&apos;ll get back to you within 24 hours.
        </p>
      </motion.div>
            ...
            >
              ...
              <div className="grid gap-5 sm:grid-cols-2">
                <Field label="Loan Amount (₹)" icon={HiOutlineCurrencyRupee} error={errors.loanAmount}>
                  <input type="text" value={form.loanAmount} onChange={(e) => set("loanAmount", e.target.value.replace(/\D/g, ""))} placeholder="1000000" className={inputCls} />
                </Field>
                <Field label="Annual Turnover (₹)" icon={HiOutlineCurrencyRupee} error={errors.annualTurnover}>
                  <input type="text" value={form.annualTurnover} onChange={(e) => set("annualTurnover", e.target.value.replace(/\D/g, ""))} placeholder="5000000" className={inputCls} />
                </Field>
              </div>
              <div className="grid gap-5 sm:grid-cols-2">
                <div className="grid gap-1.5">
                  ...
                  <div className="relative">
                    <HiOutlineOfficeBuilding className="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
                    <select value={form.businessType} onChange={(e) => set("businessType", e.target.value)} className={inputCls + " appearance-none"}>
