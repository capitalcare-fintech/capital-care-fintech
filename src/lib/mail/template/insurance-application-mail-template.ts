type BuildInsuranceApplicationEmailTemplateParams = {
...
export function buildInsuranceApplicationEmailTemplate(
  params: BuildInsuranceApplicationEmailTemplateParams,
): string {
  ...
  return `
    <div style="font-family:Arial,sans-serif;max-width:700px;margin:auto;border:1px solid #e2e8f0;border-radius:10px;overflow:hidden">
      <div style="padding:18px 22px;background:#0f172a;color:#fff">
        <h2 style="margin:0">Insurance Application Submitted</h2>
        <p style="margin:8px 0 0;opacity:.85">Application ID: ${escapeHtml(
          params.applicationId,
        )}</p>
      </div>
      <div style="padding:18px 22px;background:#f8fafc">
        <p><strong>Insurance Type:</strong> ${escapeHtml(params.insuranceTypeLabel)}</p>
